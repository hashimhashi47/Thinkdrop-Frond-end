...
export default function Profile() {
  ...
  const [tabLoading, setTabLoading] = useState(false); // Tab content load
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setUser(profileData);
        // Initial fetch for the default tab (writings)
        fetchWritings(0);
      } catch (error) {
  ...
  // Fetch Logic
  const fetchWritings = async (offset) => {
    try {
      if (offset === 0) setTabLoading(true);
      const mappedPostsData = await postService.getUserPosts(LIMIT, offset);

      if (mappedPostsData.length < LIMIT) setPostsHasMore(false);

      setPosts(prev => offset === 0 ? mappedPostsData : [...prev, ...mappedPostsData]);
      setPostsOffset(offset + LIMIT);
    } catch (error) {
  ...
  const fetchFollowers = async (offset) => {
    try {
      if (offset === 0) setTabLoading(true);
      const data = await userService.getFollowers(currentUserId, LIMIT, offset);

      if (data.length < LIMIT) setFollowersHasMore(false);

      setFollowers(prev => offset === 0 ? data : [...prev, ...data]);
      setFollowersOffset(offset + LIMIT);
    } catch (error) {
  ...
  const fetchFollowing = async (offset) => {
    try {
      if (offset === 0) setTabLoading(true);
      const data = await userService.getFollowing(currentUserId, LIMIT, offset);

      if (data.length < LIMIT) setFollowingHasMore(false);

      setFollowing(prev => offset === 0 ? data : [...prev, ...data]);
      setFollowingOffset(offset + LIMIT);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (activeTab === "writings" && posts.length === 0 && postsHasMore) {
      fetchWritings(0);
    } else if (activeTab === "followers" && followers.length === 0 && followersHasMore) {
      fetchFollowers(0);
    } else if (activeTab === "following" && following.length === 0 && followingHasMore) {
      fetchFollowing(0);
    }
  }, [activeTab, user]);
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentNode.innerHTML =
                        '<User size={64} class="text-gray-300" />';
                    }}
                  />
            ...
            <div className="flex-1 w-full text-center md:text-left">
              ...
              <div className="flex items-center gap-3 justify-center md:justify-start mb-6">
                <p className="text-brand-primary font-semibold text-sm uppercase tracking-wider">
                  {extendedData.level}
                </p>
                <span className="text-gray-600">•</span>
                <Link
                  to="/edit-profile"
                  className="flex items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-white transition-colors bg-[#1E1E2E] px-3 py-1 rounded-full border border-white/5 hover:border-white/20"
                >
              ...
              <div className="grid grid-cols-3 gap-4 max-w-md mx-auto md:mx-0">
                <button
                  ...
                  className="bg-[#1E1E2E] rounded-2xl p-4 flex flex-col items-center group cursor-pointer hover:bg-white/5 transition-colors border border-transparent hover:border-brand-primary/20"
                >
                  <FileText
                    size={20}
                    className={`mb-2 transition-colors ${activeTab === "writings" ? "text-brand-primary" : "text-gray-400 group-hover:text-white"}`}
                  />
                ...
                >
                  <Users
                    size={20}
                    className={`mb-2 transition-colors ${activeTab === "followers" ? "text-brand-primary" : "text-gray-400 group-hover:text-white"}`}
                  />
                  <span className="text-xl font-bold text-white mb-0.5">
                ...
                >
                  <Bookmark
                    size={20}
                    className={`mb-2 transition-colors ${activeTab === "following" ? "text-brand-primary" : "text-gray-400 group-hover:text-white"}`}
                  />
                  <span className="text-xl font-bold text-white mb-0.5">
                  ...
                  <>
                    ...
                    {postsHasMore && (
                      <div className="text-center pt-4">
                        <button
                          onClick={() => fetchWritings(postsOffset)}
              ...
              {followers.length > 0 ? (
                <>
                  {followers.map((follower) => (
                    <div
                      key={follower.id}
                      className="bg-[#2D2D44] p-4 rounded-xl border border-white/5 flex items-center justify-between"
                    >
                  ...
                  {followersHasMore && (
                    <div className="text-center pt-4">
                      <button
                        onClick={() => fetchFollowers(followersOffset)}
                        className="px-6 py-2 bg-[#2D2D44] hover:bg-[#363654] text-gray-300 rounded-full text-sm font-medium transition-colors border border-white/5"
                      >
                        Load More Followers
                      </button>
              ...
              {following.length > 0 ? (
                <>
                  ...
                  {followingHasMore && (
                    <div className="text-center pt-4">
                      <button
                        onClick={() => fetchFollowing(followingOffset)}
                        className="px-6 py-2 bg-[#2D2D44] hover:bg-[#363654] text-gray-300 rounded-full text-sm font-medium transition-colors border border-white/5"
                      >
                        Load More Following
                      </button>
